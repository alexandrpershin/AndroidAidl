 ...
 * Target - compression of TXT files
 * */

class CompressTextFileJob(
    ...
    override fun compressFile(filePath: String) {
        if (isFileExtensionValid(filePath)) {
            ...
            job = coroutineContext.launch {
                ...
                val fakeTextFilePathResult = "/results/txts/${filePath}"

                callback?.onCompressFinished(fakeTextFilePathResult)
            }
        }
    }
